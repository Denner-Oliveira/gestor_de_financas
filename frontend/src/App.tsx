...
function App() {
  ...
  if (authenticated) {
    if (page === 'report') return <ReportPage email={email} onBack={() => setPage('dashboard')} onLogout={logout} />
    return <DashboardPage email={email} onLogout={logout} onReport={() => setPage('report')} />
  }
